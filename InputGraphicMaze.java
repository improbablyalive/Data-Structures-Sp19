...
public class InputGraphicMaze extends JFrame  
{
 ...
 int y=e.getY()-start;
        ...
        public void paintComponent(Graphics g) 
        {
		  ...
		  start+(i-1)*delta, start+j*delta, start+i*delta);
           ...
           if (show==1)
           {
              ...
              ListIterator<Point> I=Path.listIterator();
              while (I.hasNext())
              {
		 ...
		 i=(int) p.getX(); j=(int) p.getY();
           ...
           {
              ListIterator<Node> J=L.listIterator();
              while (J.hasNext())
              {
  	         Node n=(Node) J.next(); 
                 Point q=n.p; i=(int) 
    ...
    {
        int i=1; int n=B.getHowMany();
        while (n > 0) 
        {if (B.belongs(i)) {A.insert(i); n--;} i++;}
    }
